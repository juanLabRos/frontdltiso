...
export default function documentSGSI() {
        ...
        const fetchDocument = async () => {
            try {
                const response = await axios.post(`http://localhost:5000/word/create/sgsi`, {
                    userId: usuario.id,
                    nombreEmpresa: "Correcta",
                    realizadoPor: "Marcos Gonzalez",
                    revisadoPor: "Melissa Sanchez",
                    aprobadoPor: "Mario Perez",
                    estado: "finalizado"
                });
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await axios.put(`http://localhost:5000/word/sgsi/${documentId}/user/buffer/${usuario.id}`, {
                content: content
            });
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full p-10">
            <h1 className="text-2xl font-bold mb-4">Documento SGSI</h1>
            <div className="w-full max-w-4xl p-4 bg-white border border-gray-300 rounded-lg shadow-md mb-4">
                <ReactQuill
                    value={content}
